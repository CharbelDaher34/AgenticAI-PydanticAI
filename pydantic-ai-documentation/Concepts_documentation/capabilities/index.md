...
A capability is a reusable, composable unit of agent behavior. Instead of threading multiple arguments through your `Agent` constructor — [instructions](https://ai.pydantic.dev/agent/#instructions) here, [model settings](https://ai.pydantic.dev/agent/#model-run-settings) there, a [toolset](https://ai.pydantic.dev/toolsets/index.md) somewhere else, a [history processor](https://ai.pydantic.dev/message-history/#processing-message-history) on yet another parameter — you can bundle related behavior into a single capability and pass it via the capabilities parameter.

Capabilities can provide any combination of:

- **Tools** — via [toolsets](https://ai.pydantic.dev/toolsets/index.md) or [builtin tools](https://ai.pydantic.dev/builtin-tools/index.md)
- **Lifecycle hooks** — intercept and modify model requests, tool calls, and the overall run
- **Instructions** — static or dynamic [instruction](https://ai.pydantic.dev/agent/#instructions) additions
- **Model settings** — static or per-step [model settings](https://ai.pydantic.dev/agent/#model-run-settings)
...
| ---------------- | ---------------------------------------------------------------------------------------------------- | ---- |
| Thinking         | Enables model [thinking/reasoning](https://ai.pydantic.dev/thinking/index.md) at configurable effort | Yes  |
| Hooks            | Decorator-based [lifecycle hook](https://ai.pydantic.dev/hooks/index.md) registration                | —    |
| WebSearch        | Web search — builtin when supported, local fallback otherwise                                        | Yes  |
| WebFetch         | URL fetching — builtin when supported, custom local fallback                                         | Yes  |
| ImageGeneration  | Image generation — builtin when supported, custom local fallback                                     | Yes  |
| MCP              | MCP server — builtin when supported, direct connection otherwise                                     | Yes  |
| PrepareTools     | Filters or modifies [tool definitions](https://ai.pydantic.dev/tools/index.md) per step              | —    |
| PrefixTools      | Wraps a capability and prefixes its tool names                                                       | Yes  |
| BuiltinTool      | Registers a [builtin tool](https://ai.pydantic.dev/builtin-tools/index.md) with the agent            | Yes  |
| Toolset          | Wraps an AbstractToolset                                                                             | —    |
| HistoryProcessor | Wraps a [history processor](https://ai.pydantic.dev/message-history/#processing-message-history)     | —    |

The **Spec** column indicates whether the capability can be used in [agent specs](https://ai.pydantic.dev/agent-spec/index.md) (YAML/JSON). Capabilities marked **—** take non-serializable arguments (callables, toolset objects) and can only be used in Python code.

[Learn about Gateway](https://ai.pydantic.dev/gateway) builtin_capabilities.py
...
[Instructions](https://ai.pydantic.dev/agent/#instructions) and [model settings](https://ai.pydantic.dev/agent/#model-run-settings) are configured directly via the `instructions` and `model_settings` parameters on `Agent` (or AgentSpec). Capabilities are for behavior that goes beyond simple configuration — tools, lifecycle hooks, and custom extensions. They compose well, especially when you want to reuse the same configuration across multiple agents or load it from a [spec file](https://ai.pydantic.dev/agent-spec/index.md).

### Thinking

The Thinking capability enables model [thinking/reasoning](https://ai.pydantic.dev/thinking/index.md) at a configurable effort level. It's the simplest way to enable thinking across providers:
...
See [Thinking](https://ai.pydantic.dev/thinking/index.md) for provider-specific details and the [unified thinking settings](https://ai.pydantic.dev/thinking/#unified-thinking-settings).

### Hooks

The Hooks capability provides decorator-based [lifecycle hook](#hooking-into-the-lifecycle) registration — the easiest way to intercept model requests, tool calls, and other events without subclassing AbstractCapability:
...
See the dedicated [Hooks](https://ai.pydantic.dev/hooks/index.md) page for the full API: decorator and constructor registration, timeouts, tool filtering, wrap hooks, per-event hooks, and more.

### Provider-adaptive tools

WebSearch, WebFetch, ImageGeneration, and MCP provide model-agnostic access to common tool types. When the model supports the tool natively (as a [builtin tool](https://ai.pydantic.dev/builtin-tools/index.md)), it's used directly. When it doesn't, a local function tool handles it instead — so your agent works across providers without code changes.

Each accepts `builtin` and `local` keyword arguments to control which side is used:

[Learn about Gateway](https://ai.pydantic.dev/gateway) provider_adaptive_tools.py
...
PrepareTools wraps a ToolsPrepareFunc as a capability, for filtering or modifying [tool definitions](https://ai.pydantic.dev/tools/index.md) per step:

[Learn about Gateway](https://ai.pydantic.dev/gateway) prepare_tools_builtin.py
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) prefix_tools_example.py
...
A capability that provides tools returns a [toolset](https://ai.pydantic.dev/toolsets/index.md) from get_toolset. This can be a pre-built AbstractToolset instance, or a callable that receives RunContext and returns one dynamically:

[Learn about Gateway](https://ai.pydantic.dev/gateway) custom_capability_tools.py
...
For [builtin tools](https://ai.pydantic.dev/builtin-tools/index.md), override get_builtin_tools to return a sequence of AgentBuiltinTool instances (which includes both AbstractBuiltinTool objects and callables that receive RunContext).

#### Toolset wrapping

get_wrapper_toolset lets a capability wrap the agent's entire assembled toolset with a [`WrapperToolset`](https://ai.pydantic.dev/toolsets/#changing-tool-execution). This is more powerful than providing tools — it can intercept tool execution, add logging, or apply cross-cutting behavior.
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) wrapper_toolset_example.py
...
get_instructions adds [instructions](https://ai.pydantic.dev/agent/#instructions) to the agent. Since it's called once at agent construction, return a callable if you need dynamic values:

[Learn about Gateway](https://ai.pydantic.dev/gateway) custom_capability_config.py
...
Instructions can also use [template strings](https://ai.pydantic.dev/agent-spec/#template-strings) (`TemplateStr('Hello {{name}}')`) for Handlebars-style templates rendered against the agent's [dependencies](https://ai.pydantic.dev/dependencies/index.md). In Python code, a callable with RunContext is generally preferred for IDE autocomplete.

### Providing model settings

get_model_settings returns [model settings](https://ai.pydantic.dev/agent/#model-run-settings) as a dict or a callable for per-step settings.

When model settings need to vary per step — for example, enabling thinking only on retry — return a callable:

[Learn about Gateway](https://ai.pydantic.dev/gateway) dynamic_settings.py
...
| --------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| get_toolset()         | AgentToolset `\| None`         | A [toolset](https://ai.pydantic.dev/toolsets/index.md) to register (or a callable for [dynamic toolsets](https://ai.pydantic.dev/toolsets/#dynamically-building-a-toolset)) |
| get_builtin_tools()   | `Sequence[`AgentBuiltinTool`]` | [Builtin tools](https://ai.pydantic.dev/builtin-tools/index.md) to register (including callables)                                                                           |
| get_wrapper_toolset() | AbstractToolset `\| None`      | [Wrap the agent's assembled toolset](#toolset-wrapping)                                                                                                                     |
| get_instructions()    | AgentInstructions `\| None`    | [Instructions](https://ai.pydantic.dev/agent/#instructions) (static strings, [template strings](https://ai.pydantic.dev/agent-spec/#template-strings), or callables)        |
| get_model_settings()  | AgentModelSettings `\| None`   | [Model settings](https://ai.pydantic.dev/agent/#model-run-settings) dict, or a callable for per-step settings                                                               |
...
For quick, application-level hooks without subclassing, use the [`Hooks`](https://ai.pydantic.dev/hooks/index.md) capability instead.
...
| on_node_run_error | `(ctx: RunContext, *, node: AgentNode, error: BaseException) -> NodeResult`        | Handle node errors (see [error hooks](#error-hooks)) |

wrap_node_run fires for every node in the [agent graph](https://ai.pydantic.dev/agent/#iterating-over-an-agents-graph) (UserPromptNode, ModelRequestNode, CallToolsNode). Override this to observe node transitions, add per-step logging, or modify graph progression:
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) node_logging_example.py
...
See [Iterating Over an Agent's Graph](https://ai.pydantic.dev/agent/#iterating-over-an-agents-graph) for more about the agent graph and its node types.
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) prepare_tools_example.py
...
For runs with event streaming (run_stream_events, event_stream_handler, [UI event streams](https://ai.pydantic.dev/ui/overview/index.md)), capabilities can observe or transform the event stream:
...
For building web UIs that transform streamed events into protocol-specific formats (like SSE), see the [UI event streams](https://ai.pydantic.dev/ui/overview/index.md) documentation and the UIEventStream base class.
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) per_run_state.py
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) guardrail_example.py
...
[Learn about Gateway](https://ai.pydantic.dev/gateway) logging_middleware_example.py
...
Capabilities are the recommended way for third-party packages to extend Pydantic AI, since they can bundle tools with hooks, instructions, and model settings. See [Extensibility](https://ai.pydantic.dev/extensibility/index.md) for the full ecosystem, including [third-party toolsets](https://ai.pydantic.dev/toolsets/#third-party-toolsets) that can also be wrapped as capabilities.

To add your package to this page, open a pull request.

## Publishing capabilities

To make a custom capability usable in [agent specs](https://ai.pydantic.dev/agent-spec/index.md), it needs a get_serialization_name (defaults to the class name) and a constructor that accepts serializable arguments. The default from_spec implementation calls `cls(*args, **kwargs)`, so for simple dataclasses no override is needed:
...
See [Extensibility](https://ai.pydantic.dev/extensibility/index.md) for packaging conventions and the broader extension ecosystem.
