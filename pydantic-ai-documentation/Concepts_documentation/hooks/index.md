...
The Hooks capability is the recommended way to add [lifecycle hooks](https://ai.pydantic.dev/capabilities/#hooking-into-the-lifecycle) for application-level concerns like logging, metrics, and lightweight validation. For reusable capabilities that combine hooks with tools, instructions, or model settings, subclass AbstractCapability instead — see [Building custom capabilities](https://ai.pydantic.dev/capabilities/#building-custom-capabilities).
...
`wrap_node_run` hooks are called automatically by agent.run(), agent.run_stream(), and agent_run.next(), but **not** when iterating with bare `async for node in agent_run:`.
...
See [Composition](https://ai.pydantic.dev/capabilities/#composition) for details on how hooks from multiple capabilities interact.
...
See [Error hooks](https://ai.pydantic.dev/capabilities/#error-hooks) for the full pattern and recovery types.

## Triggering retries with `ModelRetry`

Hooks can raise ModelRetry to ask the model to try again with a custom message — the same exception used in [tool functions](https://ai.pydantic.dev/tools/#model-retry) and output validators.
