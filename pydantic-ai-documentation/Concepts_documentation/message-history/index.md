...
Pydantic AI provides a `history_processors` parameter on `Agent` that allows you to intercept and modify the message history before each model request. History processors can also be provided via the HistoryProcessor capability.
...
- When creating new messages in a processor that should be part of the current run, use a [context-aware processor](#runcontext-parameter) and set `run_id=ctx.run_id` on the new message.
